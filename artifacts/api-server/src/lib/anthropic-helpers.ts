...
export async function generateRagQueries(
  ...
  toolDesc: { name: string; vendor: string; shortDescription: string; longDescription: string },
): Promise<string[]> {
...
Generate the JSON object now.`;
  ...
  // Always merge profile-derived queries with LLM queries so personal docs hit
  // even when the LLM focuses solely on tool-side terminology.
  const merged: string[] = [];
  const seen = new Set<string>();
  for (const q of [...profileQueries, ...llmQueries]) {
