...
### RAG
- Documents are chunked by `artifacts/api-server/src/lib/chunker.ts` — a token-aware (~500 target, 800 cap, ~50 overlap), structure-aware chunker that prefers Markdown headings, then pseudo-headings ("Chapter 4:", "Section II", "1.2.3"), then blank lines, then sentences before falling back to a hard char split. Each chunk row carries `tokenCount` and a short `headingTrail` (e.g. "FM 3-21.8 > Ch. 4 > Reconnaissance").
- Embeddings are computed in-process via `@xenova/transformers` running `Xenova/all-MiniLM-L6-v2` (384-dim, quantized int8, ~25 MB). No API key required. The model auto-downloads to `node_modules/@xenova/transformers/.cache` on first boot and is warmed on startup. Helper: `artifacts/api-server/src/lib/embeddings.ts`. The chunk-write path (`artifacts/api-server/src/lib/chunk-ingest.ts`) embeds + persists in one pass.
- Stored in pgvector: `doc_chunks.embedding vector(384)` with an HNSW cosine index. Each row also records `embedding_model` and `embedding_dim` so Task #22 can detect a model change later.
